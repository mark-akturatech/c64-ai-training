...
## Summary
Displays two sprites (multicolor ship and single-color asteroid) with full configuration workflow. For each sprite: sets global multicolor palette ($D025/$D026), auto-detects sprite mode by testing byte 63 high nibble via BIT instruction, sets data pointer ($07F8/$07F9), individual color ($D027/$D028), enables via $D015 with ORA, and positions using $D000-$D003 with MSB handling via $D010.
...
## Techniques
- sprite mode auto-detection via BIT
- multicolor setup
- pointer calculation
