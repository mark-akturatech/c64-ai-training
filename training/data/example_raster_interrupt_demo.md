...
## Summary
Sets up raster interrupt firing at scanline 251 (bottom border area). Disables CIA interrupts, clears $D011 bit 7 for raster lines < 256, installs custom handler at $0314/$0315. Handler increments border color ($D020) for a visible flash, uses nested delay loops to hold the color, then restores it. Main loop fills screen with incrementing characters for visual feedback.
