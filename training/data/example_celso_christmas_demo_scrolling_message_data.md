...
## Summary
Defines scrolling message text string with metadata for horizontal scroll display. Stores message as ASCII text, message length, horizontal raster offset (starting at 7 for pixel-level scroll), and current message offset for tracking scroll position. Values are updated dynamically by the main demo's interrupt routines.
