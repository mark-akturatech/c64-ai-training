...
## Summary
Animates two sprites: ship moves left-to-right with wrapping at screen edge, asteroid moves top-to-bottom with reset at bottom. Uses stack-based parameter passing where movement amount is pushed before JSR, then popped inside the subroutine after saving the return address. Implements 16-bit X position arithmetic with carry flag for MSB overflow handling. Synchronized to scanlines via wait macro.
