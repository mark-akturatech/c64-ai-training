...
## Summary
Scrolls only the bottom text row (lines 242-249) while the rest of the screen remains static. Uses $D016 hardware scroll bits for smooth 8-pixel sub-character scrolling. After every 8 pixels, the character buffer is shifted left and a new character is inserted from the message string. Color animation is applied to leading/trailing scroll edges.
