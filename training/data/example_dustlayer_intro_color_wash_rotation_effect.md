...
## Summary
Implements rotating color gradient animation on two screen rows. Saves the last color from each 40-byte table, shifts all colors left by one position, then places the saved color at the start. Two separate cycles run simultaneously: one forward, one backward, creating a flowing wave effect. Modified colors are written to Color RAM rows each frame.
