# Example: SID Music Player with Visualization

## Summary
Loads SID music data via RLE decompression, initializes player at $1000, and calls play routine ($1003) every frame via raster interrupt. Visualizes CPU load by setting $D020 border color during music update, showing how many raster lines the music routine consumes.

## Key Registers
- $D012 - VIC-II raster line - interrupt at line 25
- $D019 - VIC-II interrupt status - acknowledged
- $D020 - VIC-II border color - set to 1 during music (CPU load indicator)
- $DC00/$DC01 - CIA keyboard ports - exit detection
