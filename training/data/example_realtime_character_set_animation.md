...
## Summary
Creates animated patterns by redirecting VIC-II to a custom charset at $2000 via $D018, filling the screen with character code 0, then inverting (EOR #$FF) individual charset bytes each frame in a 40-byte rotating cycle. The character data modification is instantly visible on screen since all characters reference the same charset data. This technique is foundational for DYCP effects.
...
## Techniques
- custom charset redirection
- real-time charset modification
- bit inversion animation
- raster sync
