...
## Summary
Decrements sprite X position ($D000) each frame with 9th-bit toggle ($D010) when X underflows past zero. Animates sprite shape by cycling through 16 frames via the sprite pointer at $07F8. Uses a delay toggle to animate every other frame. Resets frame counter when animation cycle completes.
...
## Techniques
- sprite pointer animation
- 9-bit X position handling
- frame skipping
