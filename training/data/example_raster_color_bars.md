...
## Summary
Creates animated color bars by generating sine wave lookup tables at startup, then using those values to position colored bars on screen. The main loop clears a raster buffer, calculates bar positions using dual sine table lookups averaged together, and plots color values from pre-defined bar palette tables. During display, $D012 is polled in a tight loop to synchronize color writes to $D020 with each scanline. A NOP/INX toggle via self-modifying code alternates timing every 256 frames.
