# Example: Full Keyboard Matrix Scanner
#
# Implements complete keyboard matrix scanning by testing all 8 rows
# sequentially. Configures CIA port A ($DC00) as output for row selection and
# port B ($DC01) as input for column reading. Scans each row with patterns $FE
# through $7F, reads column data, and uses 8 lookup tables to convert key
# positions to character codes. Displays detected character at screen position.
# Disables keyboard timer ($DC0E) during scanning.
#
# Key Registers:
#   $DC00 - CIA port A - row scan patterns ($FE, $FD, $FB, $F7, $EF, $DF, $BF, $7F)
#   $DC01 - CIA port B - column data readback
#   $DC02/$DC03 - CIA data direction registers - port I/O configuration
#   $DC0E - CIA control register - timer A enable/disable
#
# Techniques: complete keyboard matrix scanning, lookup tables, row-by-row column testing
# Hardware: CIA
#
