# Example: Single Line Hardware Scroll
#
# Scrolls only the bottom text row (lines 242-249) while the rest of the screen remains static.
# Uses $D016 hardware scroll bits for smooth 8-pixel sub-character scrolling. After every 8
# pixels, the character buffer is shifted left and a new character is inserted from the message
# string. Color animation is applied to leading/trailing scroll edges.
#
# Key Registers:
#   $D016 - VIC-II control 2 - hardware scroll bits for X position
#   $D012 - VIC-II raster line - triggers at 0 (top) and 242 (scroll zone)
#   $D019 - VIC-II interrupt status - acknowledged
#   $07C0-$07E7 - Screen RAM bottom row - shifted left for scroll
#   $DBC0-$DBE7 - Color RAM bottom row - animated colors
#
# Techniques: hardware scrolling, character buffer shifting, per-row scroll isolation, color animation
# Hardware: VIC-II
#
