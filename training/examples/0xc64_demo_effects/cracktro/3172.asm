// Example: Multi-Part Cracktro with DYCP
//
// Comprehensive cracktro featuring raster-interrupt driven sprite splits, color cycling, DYCP
// scroller, logo transitions, and raster bars. Uses chained raster interrupts at multiple
// scanlines by updating $0314/$0315 and $D012 within each handler. Manages 8 sprites via position
// registers $D000-$D00F with MSB handling through $D010. Implements sine-table driven animations
// for smooth motion and color transitions through $D800 color RAM.
//
// Key Registers:
//   $0314/$0315 - IRQ vector - changed mid-frame for interrupt chaining
//   $D012 - VIC-II raster line - trigger for multiple interrupt points
//   $D019 - VIC-II interrupt status - acknowledged in each handler
//   $D015 - VIC-II sprite enable - enables/disables sprites per split
//   $D000-$D00F - VIC-II sprite positions - X/Y for sprites 0-7
//   $D010 - VIC-II sprite X MSB - extends X coordinates beyond 255
//   $D018 - VIC-II memory setup - switched for custom font at $3000
//   $D020/$D021 - VIC-II border/background - color cycling effects
//   $DC0D/$DD0D - CIA interrupt control - disabled during setup
//
// Techniques: raster interrupt chaining, sprite multiplexing, DYCP scroller, sine table animation, color cycling, RLE decompression
// Hardware: VIC-II, CIA
...
// Compiler: KickAssembler

                        // zero page registers

.label REG_ZERO_02             = $02
.label REG_ZERO_03             = $03
.label REG_ZERO_04             = $04
.label REG_ZERO_2A             = $2a
.label REG_ZERO_52             = $52
.label REG_ZERO_FB             = $fb                // reserve for music player
.label REG_ZERO_FC             = $fc                // reserve for music player
.label REG_ZERO_FD             = $fd                // reserve for music player
.label REG_ZERO_FE             = $fe                // reserve for music player
...
.label C_SCREEN_BANK_0         = $0400              // screen RAM
.label C_DYCP_HIGH_ROW         = $05b8              // screen RAM - dycp row 1
.label C_DYCP_LOW_ROW          = $05e0              // screen RAM - dycp row 2
.label C_SPRITE_PTRS_BANK_3    = $07f8              // location of sprite pointers in bank 3
.label C_MUSIC_LOCATION        = $1000              // location of music
.label C_MUSIC_INIT            = $1000              // music init routine
.label C_MUSIC_PLAY            = $1003              // music player routine
.label C_SPRITES_BANK_3        = $2000              // start of sprite data location
.label C_CHARSET               = $3000              // storage for character set
.label C_SCREEN_RAM_BANK_2     = $4400              // screen memory in VIC bank 2
.label C_SPRITE_PTRS_BANK_2    = $47f8              // location of sprite pointers in bank 2
.label C_SPRITES_BANK_2        = $6000              // storage for sprites in VIC bank 2
.label C_INFLATE_CACHE         = $6d00              // storage for data inflation at startup
.label C_TEMPORARY_DATA        = $c000
.label C_SCROLLER_BAR_COLOURS  = $c000              // scroller raster bar colours
.label C_SCROLL_TRANS_INDEX    = $c0d0              // scroller transition in step index
.label C_TEXT_GLOW_SEQUENCE    = $c0e0              // credit text colout sequence
.label C_ANIM_BAR_TABLE        = $c100              // animated raster bar render target
.label C_ANIM_SCROLL_TABLE     = $c120              // animates raster bar scroll offset table
.label C_ANIM_BAR_COLOURS      = $c140              // colours used in animated raster bars
.label C_ANIM_BAR_TRAN_INDEX   = $c200              // transition in index for animated raster bars
.label C_SCROLL_SINE_INDEX     = $c201              // scroller height sine index
.label C_SCROLL_MESSAGE_INDEX  = $c202              // scroller character index
.label C_TEXT_WAVE_SINE_INDEX  = $c203              // text wave sine index
.label C_SPRITE_SPLIT_INDEX    = $c205              // intro transition sprite split index (1 byte)
.label C_INTRO_CLEAR_COUNTER   = $c206              // intro transition sprite row counter (1 byte)
.label C_SPLIT_INDEX           = $c207              // final intro transition split index (1 byte)
.label C_SPLIT_SEQUENCE_INDEX  = $c208              // final intro transition seq index (7 bytes)
.label C_SPLIT_SEQUENCE_ACTIVE = $c20f              // final intro transition active seq index (7 bytes)
.label C_SMOOTHING_1           = $c217              // smoothing counter (1 byte)
.label C_SCROLL_BUFFER_LOW     = $c240              // scroller source char low byte table (40 bytes)
.label C_SCROLL_BUFFER_HIGH    = $c280              // scroller source char high byte table (40 bytes)
.label C_SCROLL_BUFFER_HEIGHT  = $c2d2              // scroller height table (40 bytes)
.label C_LOGO_COLOURS          = $c300              // logo colour table
.label C_LOGO_COL_INDEX_1      = $c340              // first logo colour index
.label C_LOGO_COL_INDEX_2      = $c341              // second logo colour index
.label C_LOGO_TRANS_STEP       = $c342              // logo transition step (0 - 2)
.label C_LOGO_TRANS_ACTIVE     = $c343              // logo transition is active
.label C_LOGO_TRANS_DELAY      = $c344              // delay between logo transitions
.label C_SCROLLER_ACTIVE       = $c345              // text scroller active
.label C_TEXT_BARS_ACTIVE      = $c346              // animated text bars active
.label C_SCROLL_MESSAGE        = $c400              // scroller message
.label C_COLOUR_RAM            = $d800              // colour ram
...
relocate_font_n_sprites:
                        lda C_INFLATE_CACHE, x
                        sta C_CHARSET, x
...
relocate_scroll_message:
                        lda C_INFLATE_CACHE + $ef9, x
...
relocate_text_glow_cols:
                        lda colour_glow_sequence, x
                        sta C_TEXT_GLOW_SEQUENCE, x
                        dex
                        bpl relocate_text_glow_cols

                        ldx #023                                // relocate scroller raster colours
relocate_scroll_colours:
                        lda scroller_bar_colours, x             // first 14 bytes already defined as 0
...
intro_sprite_split_done:
                        ldy #248                        // all sprite splits performed
                        sty REG_RASTERLINE
...
logo_split_toprow_done:
                        lda logo_split_top_row, y       // next interrupt before sprites completely drawn
                        sta REG_RASTERLINE
...
update_scroller_buffer:
                        ldy logo_index                  // only process during second logo rendering
...
append_next_scroll_char:
                        ldy #$30                        // default high source byte
                        cmp #031
                        bcc calc_character_data_pos
                        ldy #$31                        // adjust high source byte
                        and #031
calc_character_data_pos:
                        asl                             // x8 to get offset
                        asl
                        asl
...
continue_to_next_split:
                        lda logo_split_top_a, y         // need to render bottom logo
                        sta REG_RASTERLINE
...
scroll_top_first_colour:
                        lda C_SCROLLER_BAR_COLOURS    // set first colour - bad line
                        sta REG_BORCOLOUR
                        sta REG_BGCOLOUR
                        nop
                        nop
...
scroll_bot_first_colour:
                        lda C_SCROLLER_BAR_COLOURS + $07  // set first colour - bad line
                        sta REG_BORCOLOUR
                        sta REG_BGCOLOUR
                        nop
                        nop
...
update_raster_bar_anim:
                        lda #001                        // reset smoothing
                        sta animated_bar_smoothing

                        ldx #007                        // clear raster bar render table
                        lda #000
clear_raster_bar_table:
                        sta C_ANIM_BAR_TABLE + 4, x
...
cycle_scroller_colours:
                        lda C_TEXT_BARS_ACTIVE          // cycle scroller colours when active
                        beq rasterbars_update_done

                        ldy #038                        // cycle colour ram right
shift_scroller_colours:
                        lda $d9b8, y
                        sta $d9b9, y
                        lda $d9e0, y
                        sta $d9e1, y
...
load_special_scroll_col:
                        cpx #010                        // grab next highlight colour
                        bcs use_default_scroll_col
                        lda scroller_colour_cycle, x
                        jmp use_default_scroll_col + 2

use_default_scroll_col:
                        lda #006                        // default scroller colour
                        sta $d9b8                       // insert next colour
                        sta $d9e0

                        stx scroll_colours_delay

rasterbars_update_done:
                        ldy #130
                        sty REG_RASTERLINE
                        lda #<render_scroller_top
                        ldx #>render_scroller_top
