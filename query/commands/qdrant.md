...
```bash
npx tsx query/src/index.ts --limit 4 "<query>"
```
...
```bash
npx tsx query/src/index.ts --limit 8 "<query>"
```
...
```bash
npx tsx query/src/index.ts --limit 4 "$DC0D CIA interrupt control"
```
...
```bash
npx tsx query/src/index.ts --limit 4 "<topic from the reference description>"
```
