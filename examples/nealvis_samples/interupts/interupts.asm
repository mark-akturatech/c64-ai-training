# Example: Raster Interrupt Demo
#
# Sets up raster interrupt firing at scanline 251 (bottom border area). Disables
# CIA interrupts, clears $D011 bit 7 for raster lines < 256, installs custom
# handler at $0314/$0315. Handler increments border color ($D020) for a visible
# flash, uses nested delay loops to hold the color, then restores it. Main loop
# fills screen with incrementing characters for visual feedback.
#
# Key Registers:
#   $DC0D/$DD0D - CIA interrupt control - disabled with $7F
#   $D011 - VIC-II control 1 - bit 7 cleared for scanline masking
#   $D012 - VIC-II raster line - trigger set to 251
#   $D01A - VIC-II interrupt mask - raster IRQ enabled ($01)
#   $0314/$0315 - IRQ vector - custom handler
#   $D019 - VIC-II interrupt status - acknowledged
#   $D020 - VIC-II border color - flashed in handler
#
# Techniques: raster interrupts, CIA disabling, border color timing, nested delay loops
# Hardware: VIC-II, CIA
#
