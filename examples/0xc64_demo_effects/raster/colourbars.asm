# Example: Raster Interrupt Color Bars
#
# Generates scrolling color bars using raster interrupts. Each interrupt sets $D021 background
# color from a lookup table with variable-width timing delays. Smooth animation through index
# cycling with AND #15 wrapping across a 16-entry color palette.
#
# Key Registers:
#   $D012 - VIC-II raster line - triggers at lines 49 and 250
#   $D019 - VIC-II interrupt status - acknowledged via ASL
#   $D021 - VIC-II background color - set from color table
#   $D01A - VIC-II interrupt mask - raster interrupt enabled
#   $DC0D/$DD0D - CIA interrupt control - disabled
#
# Techniques: raster interrupts, lookup table animation, timing delays, index cycling
# Hardware: VIC-II, CIA
#
