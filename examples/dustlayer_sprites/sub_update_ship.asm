# Example: Sprite Animation and X-Movement
#
# Decrements sprite X position ($D000) each frame with 9th-bit toggle
# ($D010) when X underflows past zero. Animates sprite shape by cycling
# through 16 frames via the sprite pointer at $07F8. Uses a delay toggle
# to animate every other frame. Resets frame counter when animation cycle
# completes.
#
# Key Registers:
#   $D000 - VIC-II sprite 0 X position - decremented each frame
#   $D010 - VIC-II sprite X MSB - toggled on X underflow
#   $07F8 - Sprite 0 data pointer - incremented for frame animation
#
# Techniques: sprite pointer animation, 9-bit X position handling, frame skipping
# Hardware: VIC-II
